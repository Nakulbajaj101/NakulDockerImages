...
For **Celery cleanup and build**

    If you just want to start:

    bash celeryExecutorScript.sh start 

    If you want to clean up local system and start:

    bash celeryExecutorScript.sh 

For **CeleryExecutor**

    docker-compose -f docker-compose-CeleryExecutor.yml up -d

For **Recreating and Running Celery**

    docker-compose -f docker-compose-CeleryExecutor.yml up --force-recreate -d

For **Local cleanup and build**

     If you just want to start:

    bash localExecutorScript.sh start 

    If you want to clean up local system and start:

    bash localExecutorScript.sh 

For **Recreating and Running Local**

    docker-compose -f docker-compose-LocalExecutor.yml up --force-recreate -d
...
By default, docker-airflow runs Airflow with **SequentialExecutor** :
    ...
    docker run -d -p 8081:8080 puckel/docker-airflow webserver
...
`LOAD_EX=n`
    ...
    docker run -d -p 8081:8080 -e LOAD_EX=y puckel/docker-airflow
...
- Airflow: [localhost:8080](http://localhost:8081/)
